...
bool MobileApp::LoadConfig() {
  ConfigParser ini;
  if (ini.Load("/home/luotang.me/conf/mobile.conf") != 0) {
    return false;
  }
...
int32_t MobileApp::Start() {
  ...
  std::vector<StackArg> threadArgs;
  threadArgs.push_back(std::move(StackArg("ATCommand")));
  threadArgs.push_back(std::move(StackArg("ATDialing")));
  threadArgs.push_back(std::move(StackArg("ATState")));
  threadArgs.push_back(std::move(StackArg("MobileRPC")));
  stack_->SetStackArgs(threadArgs);
  stack_->StartStack();
  ...
  InetAddress addr("0.0.0.0", 9999, AF_INET, SOCK_STREAM);
  FastRpcServer* server = new FastRpcServer(stack_->GetOneLoop(), addr);

  MobileServiceImpl service_impl(1);
  server->HookService(&service_impl);
