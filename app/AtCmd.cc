...
int ATCmdManager::setNewPinCode(const std::string & oldcode, const std::string & newcode)
{
    char cmd[MAX_AT_CMD_LEN] = { 0 };
    snprintf(cmd, sizeof(cmd), "AT+CPIN=\"%s\",\"%s\"", oldcode.c_str(), newcode.c_str());
    int ret = sendCommand_(cmd, nullptr);
    if (ret != 0) {
        //error info
    }
...
int ATCmdManager::setNetSearchType()
{
    ...
    //     rc = at_send_command("AT^SYSCFGEX=\"030201\",3FFFFFFF,1,2,7FFFFFFFFFFFFFFF,,", nullptr);
...
bool ATCmdManager::setSMSFormat()
{
    ...
    if (AT_SUCCESS == sendCommand_(cmd, nullptr))
        return true;
...
bool ATCmdManager::getAPNS(void) 
{
    ...
    for (p_cur = rsp->p_intermediates; p_cur != nullptr;
         p_cur = p_cur->p_next) {
