...
*
**************************************************************************/
#ifndef _AT_CHANNEL_H_
#define _AT_CHANNEL_H_
 ...
 */
typedef std::function<void(const char *line, const char *smsPdu)> ATUnsolHandler;
...
{
    public:
        ATChannel(ATUnsolHandler h1, ATReaderClosedHandler h2, ATReaderTimeOutHandler h3);
        ~ATChannel();

        int handShake(void);

        int sendCommandSms(const char *cmd, const char *pdu,
        ...
        void registSendCmdCb(ATCmdManager *acm)
        {
            if (acm != nullptr) {
                acm->setSendCommandCb(
                    std::bind(&ATChannel::sendCommand, this,
                        std::placeholders::_1, std::placeholders::_2),
                    std::bind(&ATChannel::sendCommandNumeric, this,
                        std::placeholders::_1, std::placeholders::_2),
    ...
    private:
        ...
        int sendCommandNoLock(const char *cmd, ATCmdType type,
                    const char *rspPrefix, const char *smsPdu,
                    uint64_t timeoutMsec, ATResponse **outRsp);

        int sendCommandFull(const char *cmd, ATCmdType type,
                    const char *rspPrefix, const char *smsPdu,
                    uint64_t timeoutMsec, ATResponse **outRsp);
        ...
        int sendCommandSingleLine(const char *cmd, 
                                const char *rspPrefix,
                                ATResponse **outRsp);
        int sendCommandNumeric(const char *cmd, ATResponse **outRsp);

        int sendCommandMultiLine(const char *cmd,
                                const char *rspPrefix,
                                ATResponse **outRsp);
