...
typedef std::function<int(const char *, const char *, ATResponse **)> SendCmdSingleLineCb;
typedef std::function<int(const char *, const char *, ATResponse **)> SendCmdMultiLineCb;
...
class ATCmdManager {
  public:
    ...
    virtual int setNewPinCode(const std::string & oldcode, const std::string & newcode);
    ...
    void setSendCommandCb(SendCmdCb send, SendCmdNumbericCb sendNum,
          SendCmdSingleLineCb sendSLine, SendCmdMultiLineCb sendMLine,
          AllocResponceCb allocRsp, FreeResponceCb freeRsp,
          GetCmeError getErr)
    {
      sendCommand_ = send;
