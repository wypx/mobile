...
{
    new ModemInfo(USB_MODEM_IDVENDOR_LONGCHEER, USB_MODEM_IDPRODUCT_U8300,  "U8300",
        MODEM_LONGSUNG, 1, 3, 2, 2),
    new ModemInfo(USB_MODEM_IDVENDOR_LONGCHEER, USB_MODEM_IDPRODUCT_U8300W, "U8300W",
        MODEM_LONGSUNG, 1, 3, 2, 2),
    new ModemInfo(USB_MODEM_IDVENDOR_LONGCHEER, USB_MODEM_IDPRODUCT_U8300C, "U8300C",
        MODEM_LONGSUNG, 2, 1, 3, 2),
    new ModemInfo(USB_MODEM_IDVENDOR_LONGCHEER, USB_MODEM_IDPRODUCT_U9300C, "U9300C",
        MODEM_LONGSUNG, 2, 1, 3, 2),
    new ModemInfo(USB_MODEM_IDVENDOR_HUAWEI, USB_MODEM_IDPRODUCT_ME909u_521, "ME909u_521",
        MODEM_HUAWEI, 4, 2, 1, 0),
    new ModemInfo(USB_MODEM_IDVENDOR_HUAWEI, USB_MODEM_IDPRODUCT_ME909s_821, "ME909s-821",
        MODEM_HUAWEI,  4, 2, 1, 0),
    new ModemInfo(USB_MODEM_IDVENDOR_HUAWEI, USB_MODEM_IDPRODUCT_ME909s_121, "ME909s_121",
        MODEM_HUAWEI,  4, 2, 1, 0)
};
...
bool Modem::checkIdSupport(const uint32_t vendorId, const uint32_t productId)
{
    for (uint32_t i = 0; i < MSF_ARRAY_SIZE(_usbModems_); ++i) {
        ...
        if (i == (MSF_ARRAY_SIZE(_usbModems_)-1))
            return false;
    }
    return true;
}
    ...
    case MODEM_HUAWEI:
        {
            ...
            sscanf(p, ":%d,%d,%d,%d,,%d,\"%s\"",
                    &tmp[0], &tmp[1], &tmp[2], &tmp[3], &tmp[4], currMode);

            switch(tmp[4]) {
                case 1: _netMode = MODE_GPRS; break;
                case 3: _netMode = MODE_WCDMA; break;
                case 4: _netMode = MODE_TDSCDMA; break;
                case 6: 
                        _netMode = (_modem->_simOper == OPERATOR_MOBILE) ? 
                                    MODE_TDLTE : MODE_FDDLTE;
                    break;
                default: break;
            }
        }
